...
extern SPI_HandleTypeDef hspi6;  // 使用main.c中定义的SPI句柄
...
//显示16位十六进制，固定4位
void  LCD_DisplayHex( uint16_t x, uint16_t y, uint16_t number, uint8_t len)
{
	char Number_Buffer[15];
	if(LCD.ShowNum_Mode == Fill_Zero)
	{
		sprintf(Number_Buffer,"%0*X",len,number);
	}
	else
	{
		sprintf(Number_Buffer,"%*X",len,number);
	}

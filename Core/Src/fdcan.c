...
void MX_FDCAN1_Init(void)
{
  ...
  sFilterConfig1.FilterIndex = 0;                     // 过滤器索引0
  sFilterConfig1.FilterType = FDCAN_FILTER_MASK;      // 掩码模式
  sFilterConfig1.FilterConfig = FDCAN_FILTER_TO_RXFIFO0; // 进FIFO0
  sFilterConfig1.FilterID1 = 0x00000000;               // 接收所有ID
  sFilterConfig1.FilterID2 = 0x00000000;               // 掩码为0
...
void MX_FDCAN3_Init(void)
{
  ...
  sFilterConfig3.FilterIndex = 0;                     // 过滤器索引0
  sFilterConfig3.FilterType = FDCAN_FILTER_MASK;      // 掩码模式
  sFilterConfig3.FilterConfig = FDCAN_FILTER_TO_RXFIFO0; // 进FIFO0
  sFilterConfig3.FilterID1 = 0x000;               // 接收所有ID
  sFilterConfig3.FilterID2 = 0x000;               // 掩码为0
...
/* USER CODE BEGIN 1 */
// FDCAN发送扩展帧：id=29位扩展ID，data=数据，len=长度(1-8)
uint8_t FDCAN_Send_ExtFrame(FDCAN_HandleTypeDef *hfdcan, uint32_t id, uint8_t *data, uint8_t len)
{
  ...
  tx_header.FDFormat = FDCAN_CLASSIC_CAN;    // 普通CAN模式
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
  ...
  if((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != 0)
  {
    while(HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &rx_header, data) == HAL_OK)
    {
      ...
      {
        uint8_t id = (uint8_t)((rx_header.Identifier >> 8) & 0xFF); // 提取电机ID（位于ID的特定位）
        // 解析电机状态
        ...
        if (hfdcan->Instance == FDCAN1) {
          ...
          motor_status_fault |= (((rx_header.Identifier >> 16) & 0x3F) != 0) << (id - l_leg_pitch + 6);
